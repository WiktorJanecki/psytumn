...
use crate::{input::InputState, Level, texturemanager::TextureManager, render::Camera, components};
...
pub fn update(state: &mut MenuState, dt: f32, input_state: &mut InputState, level: &mut Level){
    if !state.update_started{
        ...
        state.world.spawn((
            components::Sprite{filename: "res/bullet.png", size: UVec2::new(128 - 64,128 - 64)},
    ...
    for (_id, transform) in state.world.query_mut::<With<&mut components::Transform, &components::CameraTarget>>(){
...
pub fn spawn_button(world: &mut hecs::World, texture: &'static str, index: usize){
    let spacing = 256;
    world.spawn((
        components::Sprite{filename: texture, size: UVec2::new(128,128)},
